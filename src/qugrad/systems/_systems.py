...
class ExpValCustom:
    ...
    switching_function: Any
    """
    The value of the switching function multiplied by :attr:`dt` evaluated at
    ...
    def run(self, ctrl_amp):
        ...
        def grad(upstream): return self.switching_function
        return tf.math.real(tf.constant(E, dtype=tf.complex128)), grad # @tf.custom_gradient removes the second returned variable

class GateInfidelityCustom():
    """
    A class implimenting :meth:`QuantumSystem.evolved_gate_infidelity()` with
    a `TensorFlow <https://www.tensorflow.org>`__ gradient.
    """
    
    system: "QuantumSystem"
    "The system in which to calculate the gate infidelity"

    dt: float
    "The integrator time step"

    target: np.ndarray[complex]
    ...
    def __init__(self,
                 ...
                 target: np.ndarray[complex]):
        """
        Initialises the class with the `system` in which to calculate the
        gate infidelity and the `target` gate calculate the gate infidelity
        from. Additionally, the integrator time step is specified.
        ...
        dt : float
            The integrator time step
        target : NDArray[Shape[``system.dim``, ``system.dim``], complex])
            The target gate to calculate the gate infidelity with respect to
        """
        self.system = system
        self.dt = dt
    ...
    def run(self, ctrl_amp):
        """
        Computes the gate infidelity from :attr:`target` of the evolution due to
        the :attr:`system` Hamiltonian generated by the specified control
        amplitudes.
        ...
        ----
        This function uses
        :meth:`~py_ste.evolvers.DenseUnitaryEvolver.gate_switching_function()`
        from `PySTE <https://PySTE.readthedocs.io>`__.
        """
        infidelity, self.switching_function = self.system.evolver.gate_switching_function(ctrl_amp.numpy().copy(), self.dt, self.target)
        self.switching_function = tf.constant(self.switching_function*self.dt, dtype=tf.complex128)
        def grad(upstream): return self.switching_function
        return tf.constant(infidelity, dtype=tf.float64), grad # @tf.custom_gradient removes the second returned variable
...
class QuantumSystem:
    ...
    *args
    ...
    def evolved_inner_product(self, *args) -> complex:
        ...
        :meth:`~py_ste.evolvers.DenseUnitaryEvolver.evolved_inner_product()`
        from `PySTE <https://PySTE.readthedocs.io>`__.
        ...
        ``*args[:-1]``
            ...
            document the parameters for this function: the same parameters as
            passed to :meth:`_pre_processing()`.
        ``args[-1]`` : NDArray[Shape[:attr:`dim`], complex]
        ...
        * :meth:`evolved_inner_product_all()`
        """
        fixed_vector: np.ndarray[complex] = args[-1]
        ctrl_amp, initial_state, dt = self.get_driving_pulses(*args[:-1])
        return self.evolver.evolved_inner_product(ctrl_amp,
                                                  initial_state,
                                                  dt,
                                                  fixed_vector)
    def evolved_inner_product_all(self, *args) -> complex:
        ...
        :meth:`~py_ste.evolvers.DenseUnitaryEvolver.evolved_inner_product_all()`
        from `PySTE <https://PySTE.readthedocs.io>`__.
        ...
        * :meth:`evolved_inner_product()`
        ...
        fixed_vector: np.ndarray[complex] = args[-1]
        ctrl_amp, initial_state, dt = self.get_driving_pulses(*args[:-1])
        return self.evolver.evolved_inner_product_all(ctrl_amp,
                                                      initial_state,
                                                      dt,
                                                      fixed_vector)
    def get_evolution(self, *args) -> np.ndarray[complex]:
        ...
        Hamiltonian defined by the control amplitudes using
        :meth:`~py_ste.evolvers.DenseUnitaryEvolver.get_evolution()`
        from `PySTE <https://PySTE.readthedocs.io>`__.
        ...
        * :meth:`propagate_all()`
        """
        ctrl_amp, _, dt = self.get_driving_pulses(*args)
        return self.evolver.get_evolution(ctrl_amp, dt)
    def evolved_gate_infidelity(self, *args) -> float:
        ...
        gate using
        :meth:`~py_ste.evolvers.DenseUnitaryEvolver.evolved_gate_infidelity()`
        from `PySTE <https://PySTE.readthedocs.io>`__.
        ...
        * :meth:`gate_gradient()`
        """
        target: np.ndarray[complex] = args[-1]
        ctrl_amp, _, dt = self.get_driving_pulses(*args[:-1])
        return self.evolver.evolved_gate_infidelity(ctrl_amp, dt, target)
    ...
    def gate_gradient(self, *args) -> tuple[float, np.ndarray[float]]:
        ...
        * :meth:`gradient()`
        ...
        with tf.GradientTape(persistent=False,
            ...
            tape.watch(args[0])
            ctrl_amp, _, dt = self._processing(*args)
            try: dt: float = dt.numpy()
            except: pass
            dt = float(dt.real)
            infidelity = GateInfidelityCustom(self, dt, target).run(ctrl_amp)
        grad = tape.gradient(infidelity, args[0])
        del tape
        
        grad = tf.convert_to_tensor(grad)
        grad: np.ndarray[float] = grad.numpy()
        grad = grad.real
        return infidelity.numpy(), grad
    def pulse_form(self,
        ...
        append : bool, optional
            Whether to prepend
            (``self._pre_processing(*pulse_function())``) or append
            (``pulse_function(*self._pre_processing())``)
            ``pulse_function``. By default ``False``.
